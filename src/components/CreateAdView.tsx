...
const CreateAdView: React.FC<CreateAdViewProps> = ({ onCreateAd, onUpdateAd, showToast }) => {
  ...
  const renderUploadStep = () => (
    <div>
            ...
            <div className="mb-4">
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                    {imagePreviews.map((preview) => (
                     ...
                     <label className="cursor-pointer aspect-square flex flex-col items-center justify-center bg-gray-100 dark:bg-tg-bg border-2 border-dashed border-gray-300 dark:border-tg-border rounded-lg hover:bg-gray-200 dark:hover:bg-tg-secondary-bg-hover">
                        <Icon icon="lucide:plus" className="h-8 w-8 text-gray-400 dark:text-tg-hint" /><span className="text-xs text-gray-500 dark:text-tg-hint mt-1">Додати</span>
        ...
        {imagePreviews.length === 0 ? (
            <label className="cursor-pointer w-full h-40 bg-gray-100 dark:bg-tg-bg border-2 border-dashed border-gray-300 dark:border-tg-border rounded-lg flex flex-col items-center justify-center hover:bg-gray-200 dark:hover:bg-tg-secondary-bg-hover">
                <Icon icon="lucide:upload-cloud" className="h-10 w-10 text-gray-400 dark:text-tg-hint" /><span className="mt-2 font-semibold">Натисніть для завантаження</span><span className="text-xs text-gray-500 dark:text-tg-hint mt-1">PNG, JPG, WEBP до 10МБ</span>
                <input type="file" className="hidden" onChange={handleImageChange} multiple />
            </label>
        ) : (
 ...
 const renderEditStep = () => {
        ...
        return (
            <div className="space-y-4">
                <p className="text-center text-sm text-gray-500 dark:text-tg-hint">Перше фото буде головним. Застосуйте AI-покращення, щоб зробити його ідеальним.</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <button onClick={() => handleImageEdit('background')} disabled={isEditingImage} className="flex items-center justify-center gap-2 p-3 bg-gray-200 dark:bg-tg-secondary-bg-hover rounded-lg hover:bg-gray-300 dark:hover:bg-tg-bg disabled:opacity-50"><Icon icon="lucide:wand-2" /><span>Замінити фон на білий</span></button>
                    <button onClick={() => handleImageEdit('enhance')} disabled={isEditingImage} className="flex items-center justify-center gap-2 p-3 bg-gray-200 dark:bg-tg-secondary-bg-hover rounded-lg hover:bg-gray-300 dark:hover:bg-tg-bg disabled:opacity-50"><Icon icon="lucide:sparkles" /><span>Покращити якість</span></button>
                </div>
  ...
  return (
    <div className="max-w-md mx-auto bg-white dark:bg-tg-secondary-bg p-6 rounded-lg shadow-xl">
        ...
        {step === 'describe' && !isEditMode && (
            <div>
                 <textarea value={userPrompt} onChange={(e) => setUserPrompt(e.target.value)} placeholder="Наприклад: 'Продаю майже новий червоний велосипед, у відмінному стані, є кілька подряпин на рамі'" className="w-full bg-gray-100 dark:bg-tg-bg p-3 rounded-lg border border-gray-300 dark:border-tg-border focus:ring-2 focus:ring-tg-link focus:outline-none" rows={4}/>
        ...
        {step === 'generating' && <div className="text-center py-8"><Spinner size="lg"/> <p className="mt-4 text-gray-500 dark:text-tg-hint">Аналізуємо фото та опис...</p></div>}
                ...
                {isEditMode && (
                    <div className="mb-4">
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                            ...
                            <label className="cursor-pointer aspect-square flex flex-col items-center justify-center bg-gray-100 dark:bg-tg-bg border-2 border-dashed border-gray-300 dark:border-tg-border rounded-lg hover:bg-gray-200 dark:hover:bg-tg-secondary-bg-hover"><Icon icon="lucide:plus" className="h-8 w-8 text-gray-400 dark:text-tg-hint" /><span className="text-xs text-gray-500 dark:text-tg-hint mt-1">Додати</span><input type="file" className="hidden" onChange={handleImageChange} multiple /></label>
                        </div>
                    </div>
                )}
                <input value={formData.title} onChange={(e) => setFormData(d => d ? { ...d, title: e.target.value } : null)} className="w-full bg-gray-100 dark:bg-tg-bg p-2 mb-2 rounded border border-gray-300 dark:border-tg-border" placeholder="Назва"/>
                <textarea value={formData.description} onChange={(e) => setFormData(d => d ? { ...d, description: e.target.value } : null)} className="w-full bg-gray-100 dark:bg-tg-bg p-2 mb-2 rounded border border-gray-300 dark:border-tg-border" rows={5} placeholder="Опис"/>
                <input value={formData.price} onChange={(e) => setFormData(d => d ? { ...d, price: e.target.value } : null)} className="w-full bg-gray-100 dark:bg-tg-bg p-2 mb-2 rounded border border-gray-300 dark:border-tg-border" placeholder="Ціна"/>
                <input value={formData.location} onChange={(e) => setFormData(d => d ? { ...d, location: e.target.value } : null)} className="w-full bg-gray-100 dark:bg-tg-bg p-2 mb-2 rounded border border-gray-300 dark:border-tg-border" placeholder="Місто"/>
                <input value={formData.category} onChange={(e) => setFormData(d => d ? { ...d, category: e.target.value } : null)} className="w-full bg-gray-100 dark:bg-tg-bg p-2 mb-2 rounded border border-gray-300 dark:border-tg-border" placeholder="Категорія"/>
